...
def get_question_success_rate_stats(db: Session):
    results = db.query(
        models.Question.id,
        models.Question.word,
        func.count(models.AnswerLog.id).label("total_attempts"),
        func.sum(cast(models.AnswerLog.is_correct, Integer)).label("correct_attempts")
    ).outerjoin(models.AnswerLog, models.Question.id == models.AnswerLog.question_id)\
     .group_by(models.Question.id, models.Question.word)\
     .all()
    ...
    for q in results:
        ...
        correct = q.correct_attempts or 0
    ...
    return [
        {"difficulty_group": "🟢 سهلة (>80%)", "question_count": easy},
        {"difficulty_group": "🟡 متوسطة (30%-80%)", "question_count": medium},
        {"difficulty_group": "🔴 صعبة (<30%)", "question_count": hard},
        {"difficulty_group": "⚪ لم تُجاب بعد", "question_count": no_attempts},
    ]